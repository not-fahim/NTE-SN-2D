...
{
    std::cout<<"Reading DOTB flux output from file: " << filename << std::endl;
    bool pincalc = false;
    ...
    inputFile.close();
    std::cout<<"Finished reading DOTB flux output from file: " << filename << std::endl;
    return dotb_output;
...
pin_data_class::pin_data_class(std::string filename)
{
    ...
    std::cout<< "num pins x: " << pin_num_x << " num pins y: " << pin_num_y << " pitch: " << pin_pitch << std::endl;
    ...
    std::cout<<"Mesh size in x: " << mesh_delxi.size() << " Mesh size in y: " << mesh_delyj.size() << std::endl;
    mesh_flux = dotb.mesh_flux;
    total_fission_rate = 0.0;
    mesh_fission_density = dotb.mesh_fission_density;
    std::cout<<"fission density matrix size: " << mesh_fission_density.size() << " x " << mesh_fission_density[0].size() << std::endl;
    ...
    std::cout << "Pin data initialized for case: " << name << std::endl;
...
void pin_data_class::calculate_pin_power()
{
    ...
    std::cout<< "first pin x min max: " << pin_x_min << " " << pin_x_max << std::endl;
    std::cout<< "first pin y min max: " << pin_y_min << " " << pin_y_max << std::endl;

    int pi = 0; //pin index x
    int pj = 0; //pin index y
    double mesh_volume;
    std::cout << "Calculating pin power for case: " << name << std::endl;
    ...
    std::cout << "Total fission rate before normalization: " << total_fission_rate << std::endl;
    normalize_vector(pin_fission_rate, total_fission_rate);
    for(pi=0; pi<pin_num_x; pi++)
    {
        for(pj=0; pj<pin_num_y; pj++)
        {
            std::cout<<pin_fission_rate[pi][pj]<<" ";
        }
        std::cout<<std::endl;
    }
